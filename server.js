...
app.post("/assigncode", async (req, res) => {
  try {
    ...
    if (barcode) {
      await CsvModel.findByIdAndUpdate(entryId, {
...
app.post("/isDelivered", async (req, res) => {
  try {
    ...
    const deliveryStatus = await CsvModel.findOneAndUpdate(
      { assignedBarcode: delivereditems },
      { isDelivered: true, isReturned: false },
      { new: true }
    );
...
app.post("/isUndelivered", async (req, res) => {
  try {
    ...
    const undeliveryStatus = await CsvModel.findOneAndUpdate(
      { assignedBarcode: undelivereditems },
      { isDelivered: false },
      { new: true }
    );
